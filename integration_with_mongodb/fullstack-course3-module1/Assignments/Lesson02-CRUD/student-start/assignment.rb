...
class Solution
  ...
  def find_by_name(fname, lname)
    #place solution here
    all(first_name: fname, last_name: lname).projection(first_name: 1, last_name: 1, number: 1, _id: 0)
  end
  ...
  def find_group_results(group, offset, limit) 
    #place solution here
    all(group: group).projection(_id: 0, group: 0).sort(secs: 1).skip(offset).limit(limit)
  end
  ...
  def find_by_letter(letter, offset, limit) 
    #place solution here
    all(last_name: {:$regex => "^#{letter.upcase}.+"}).sort(last_name: 1).skip(offset).limit(limit)
  end
  ...
  def update_racer(racer)
    #place solution here
    @coll.replace_one({_id: racer[:_id]}, racer)
  end
